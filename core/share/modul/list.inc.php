...
function modulSimpleList($config, $viewItem)
	...
	if (file_exists($config["configShare"]["configPath"].$config["modulSelect"].".inc.php")) {
		...
		$searchFilterItemCFG = $config[$modulSelect]["searchFilterCFG"];
	} else {
...
/*
		if ($_POST["s_own"] && $_POST["s_own"]=="1") {
			$s_itemValues["products_s_own"]=" checked";
			if (!empty($config[$config["modulSelect"]]["defaultFilter"])) {
				$config[$config["modulSelect"]]["defaultFilter"] .= " AND active=1 AND insertUserID=".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"];
			} else {
				$config[$config["modulSelect"]]["defaultFilter"] .= " active=1 AND insertUserID=".$_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserID"];
			}
		}
*/
	}
...
//			if ( ISSET($_SESSION["whichModul"]) ) $config[$config["modulSelect"]]["defaultFilter"] = " AND modul='".$_SESSION["whichModul"]."'";
	...
	if (file_exists($config["configShare"]["configPath"].$config["modulSelect"].".inc.php")) {
		require_once ($config["configShare"]["configPath"].$config["modulSelect"].".inc.php");
		$relatedTableCFG = $config[$modulSelect]["relatedTablesCFG"];
	} else {
		$relatedTableCFG = explode('<#>',$relatedTablesCFG);
	}
	if (!empty($relatedTableCFG))
		{
...
/*
if ($config["siteName"] == "zbads") {
//	var_dump($config[$modulSelect]["listItemCFG"]);
//	die();
}
*/
	if (file_exists($config["configShare"]["configPath"].$config["modulSelect"].".inc.php")) {
		require_once ($config["configShare"]["configPath"].$config["modulSelect"].".inc.php");
		$listItemCFG = $config[$modulSelect]["listItemCFG"];
	} else {
		$listItemCFG = explode('<#>',$listCFG);
	}
			...
			if ($config["siteName"]=="corvinus") {
				if (isset($itemValues['x_linebgcolor']) && !empty($itemValues['x_linebgcolor'])) {
					$config[$config["modulSelect"]]["bgcolor"] .= ' '.TRIM($itemValues['x_linebgcolor']);
					$config[$config["modulSelect"]."_"."bgcolor"] .= ' '.TRIM($itemValues['x_linebgcolor']);
				}
			}
			for ($listItemCFGCount=0;$listItemCFGCount<count($listItemCFG);$listItemCFGCount++)
				...
				switch ($listItemFieldCFG[0])
					...
					case "fullLineFull":
						if (!empty($itemValues[$listItemFieldCFG[3]]))
							{
							$config[$config["modulSelect"]."_x_".$listItemFieldCFG[2]]=longtextView($config,$itemValues[$listItemFieldCFG[3]]);
							}
						else
							{
							$config[$config["modulSelect"]."_x_".$listItemFieldCFG[2]]="";
...
function createChart($config) {
	...
	switch ($config["modulSelect"]) {
		case 'video':
			$diagramSQL = "SELECT IF(ISNULL(rateNumber),'NULL',rateNumber) AS name, COUNT(ISNULL(rateNumber)) AS counter FROM ".$config["modulSelect"]." ".$diagramWhereSQL." GROUP BY rateNumber";
//			$diagramSQL = "SELECT IF(ISNULL(makeYear),'NULL',makeYear) AS name, COUNT(ISNULL(makeYear)) AS counter FROM ".$config["modulSelect"]." ".$diagramWhereSQL." GROUP BY makeYear";
			...
			$diagramHead[0] = 'Pontszám';
			$diagramHead[1] = 'Mennyiség';
			$diagramItemTextPlus = ' pont';
			$diagramTotalText = 'Film pont megoszlása (Összesen: ';
			break;
		case 'recipe':
			$diagramSQL = "SELECT (SELECT name FROM recipetopic WHERE id=recipe.recipetopicID) AS name, COUNT(recipetopicID) AS counter FROM ".$config["modulSelect"]." ".$diagramWhereSQL." GROUP BY recipetopicID";
			$diagramList=$config["dbConnect"]->prepare($diagramSQL);
			$diagramList->execute();
			$diagramRecordCount = $diagramList->rowCount();
			$diagramRecords = $diagramList->fetchAll();
			...
			$diagramHead[1] = 'Mennyiség';
			$diagramItemTextPlus = '';
			$diagramTotalText = 'Recept kategória megoszlása (Összesen: ';
			break;
		case 'training':
...
/*
		$chartType = '<form method="post" name="listChartTypeform" action="">';
		$chartType .= '<select id="clickHide" class="form-control" style="width:100px;" id="chart" name="chart" onchange="this.form.submit();">';
		for( $i=0; $i<count($chartOptions); $i++ ) {
			if ($_REQUEST['chart'] == $chartOptions[$i]) {
				$chartType .= '<option value="'.$chartOptions[$i].'" selected="selected">' . $chartOptions[$i] . '</option>';
			} else { 
				$chartType .= '<option value="'.$chartOptions[$i].'">'.$chartOptions[$i] . '</option>';
			}
		}
